...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _saveSchoolFromDialog(
    ...
    try {
      ...
      print('Failed to create school: $e');
      print('$stackTrace');
      if (!mounted) {
        return;
      }
  ...
  Future<void> createSchool(
    ...
    try {
      print('Creating school: $trimmedName');
...
class _AdminDashboardData {
  ...
  int get totalXpInSystem {
    return progressByStudentId.values
        ...
        .fold<int>(0, (sum, entry) => sum + entry.xpEarned);
  }
  ...
  int totalXpForSchool(String schoolId) {
    ...
    return schoolStudents.fold<int>(0, (sum, student) {
      final entries = progressByStudentId[student.id] ?? const <StudentProgress>[];
      final xp = entries.fold<int>(0, (xpSum, entry) => xpSum + entry.xpEarned);
      return sum + xp;
    });
  }
