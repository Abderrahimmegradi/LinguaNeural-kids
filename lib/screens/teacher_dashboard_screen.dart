...
class _TeacherDashboardScreenState extends State<TeacherDashboardScreen> {
  ...
  Future<List<AppUserProfile>> _loadStudents() {
    // Keep students scoped to the current teacher.
    // If a legacy teacher has no schoolId yet, fall back to teacherId only
    ...
    try {
      ...
      // Return empty list on permission error instead of crashing
      return Future.value(const <AppUserProfile>[]);
    }
  }
  ...
  Future<_ClassSnapshot> _loadClassSnapshot(List<AppUserProfile> students) async {
    ...
    for (final progress in progressEntries) {
      totalXp += progress.fold<int>(0, (sum, item) => sum + item.xpEarned);
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const Text('Learning team'),
                const SizedBox(height: 8),
                Text('Completed lessons: ${insight.completedLessons}'),
                const SizedBox(height: 8),
                Text(
                  'Average progress: ${insight.averageProgress.toStringAsFixed(0)}%',
                ),
                const SizedBox(height: 8),
                Text('Total XP: ${insight.totalXp}'),
                const SizedBox(height: 8),
                Text('Streak: ${insight.streakDays} day(s)'),
                const SizedBox(height: 8),
                Text(
                  'Last activity: ${insight.lastActivity ?? 'No activity yet'}',
                ),
                const SizedBox(height: 16),
                if (insight.progress.isEmpty)
                  const Text('No lesson progress yet.')
                else
                  ...insight.progress.map(
                    (entry) => Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '${entry.score.toStringAsFixed(0)}% • ${entry.xpEarned} XP • ${entry.completed ? 'Completed' : 'In progress'}',
                          ),
                        ],
                      ),
                    ),
